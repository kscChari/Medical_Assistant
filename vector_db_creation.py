...
def clean_text(txt):
    pattern = r"The Merck Manual of Diagnosis & Therapy.*?full is liable for legal action\."

# How to apply it to your text string:
    cleaned_text = re.sub(pattern, "", txt, flags=re.DOTALL)   
    return cleaned_text
...
def create_vector_db(pdf_path: str):
    ...
    # Explicitly delete old DB
    if os.path.exists(persist_directory):
        shutil.rmtree(persist_directory)
        print("Deleted existing vector DB")# Load the PDF document
    ...
    test_splitter = RecursiveCharacterTextSplitter(chunk_size = 1000,
                                            chunk_overlap = 200)
    chunks = test_splitter.split_documents(doc)
